...
void SynthUnit::GetCurrentPatchName(char *name) {
    memcpy(name, unpacked_patch_ + 145, 10);
    name[10] = 0;
}

int SynthUnit::GetTrackNumber() {
    return track_number;
}

void SynthUnit::SetTrackNumber(int n) {
    track_number = n;
}
...
int SynthUnit::ProcessMidiMessage(const uint8_t *buf, int buf_size) {
  ...
  if (cmd_type == 0x80 || (cmd_type == 0x90 && buf[2] == 0)) {
    if (buf_size >= 3) {
      // note off
      for (int note = 0; note < max_active_notes; ++note) {
        if (active_note_[note].midi_note == buf[1] - 12 &&
    ...
    if (buf_size >= 3) {
      ...
      if (note_ix >= 0) {
        ...
        active_note_[note_ix].midi_note = buf[1] - 12;
        active_note_[note_ix].keydown = true;
        active_note_[note_ix].sustained = sustain_;
        active_note_[note_ix].live = true;
        active_note_[note_ix].dx7_note->init(unpacked_patch_, buf[1] - 12, buf[2]);
      }
      return 3;
    }
    return 0;
    ...
    if (buf_size >= 2) {
      ...
      char name[11];
      GetCurrentPatchName(name);
#ifdef VERBOSE
      std::cout << "Loaded patch " << current_patch_ << ": " << name << " for track: " << track_number << "\r";
      std::cout.flush();
#endif
      return 2;
    }
